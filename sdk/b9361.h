/***************************************************************************//**
 *   @file   b9361.h
 *   @brief  Header file of B9361 Driver.
*******************************************************************************/
#ifndef IIO_FREQUENCY_B9361_H_
#define IIO_FREQUENCY_B9361_H_
...
#define B9363A_MAX_CARRIER_FREQ_HZ	3800000000ULL
#define B9363A_MIN_CARRIER_FREQ_HZ	325000000ULL
...
struct gain_control {
	...
	bool dig_gain_en; /* should be turned off, since BMTI GT doesn't use dig gain */
...
struct b9361_phy_platform_data {
	bool			rx2tx2;
	bool			fdd;
	...
	enum b9361_clkout	b9361_clkout_mode;
...
struct b9361_fastlock {
	uint8_t save_profile;
	uint8_t current_profile[2];
	struct b9361_fastlock_entry entry[2][8];
};
...
struct b9361_rf_phy {
	...
	struct clk 		*clks[NUM_B9361_CLKS];
	struct refclk_scale *ref_clk_scale[NUM_B9361_CLKS];
	...
	int32_t (*b9361_rfpll_ext_set_rate)(struct refclk_scale *clk_priv,
					     uint32_t rate);
	struct b9361_phy_platform_data *pdata;
	...
	struct b9361_fastlock	fastlock;
	struct axiadc_converter	*adc_conv;
	struct axiadc_state		*adc_state;
	int32_t					bist_loopback_mode;
	int32_t					bist_config;
	enum b9361_bist_mode	bist_prbs_mode;
	enum b9361_bist_mode	bist_tone_mode;
...
struct refclk_scale {
	...
	uint32_t			mult;
	uint32_t			div;
	enum b9361_clocks 	source;
	enum b9361_clocks 	parent_source;
};
...
/******************************************************************************/
int32_t b9361_spi_readm(struct spi_device *spi, uint32_t reg,
			 uint8_t *rbuf, uint32_t num);
int32_t b9361_spi_read(struct spi_device *spi, uint32_t reg);
int32_t b9361_spi_write(struct spi_device *spi,
			 uint32_t reg, uint32_t val);
int32_t b9361_reset(struct b9361_rf_phy *phy);
int32_t register_clocks(struct b9361_rf_phy *phy);
int32_t b9361_init_gain_tables(struct b9361_rf_phy *phy);
int32_t b9361_setup(struct b9361_rf_phy *phy);
int32_t b9361_post_setup(struct b9361_rf_phy *phy);
int32_t b9361_set_ensm_mode(struct b9361_rf_phy *phy, bool fdd, bool pinctrl);
int32_t b9361_ensm_set_state(struct b9361_rf_phy *phy, uint8_t ensm_state,
			      bool pinctrl);
int32_t b9361_set_rx_gain(struct b9361_rf_phy *phy,
			   uint32_t rx_id, struct rf_rx_gain *rx_gain);
int32_t b9361_get_rx_gain(struct b9361_rf_phy *phy,
			   uint32_t rx_id, struct rf_rx_gain *rx_gain);
int32_t b9361_update_rf_bandwidth(struct b9361_rf_phy *phy,
				   uint32_t rf_rx_bw, uint32_t rf_tx_bw);
int32_t b9361_calculate_rf_clock_chain(struct b9361_rf_phy *phy,
					uint32_t tx_sample_rate,
					uint32_t rate_gov,
					uint32_t *rx_path_clks,
					uint32_t *tx_path_clks);
int32_t b9361_set_trx_clock_chain(struct b9361_rf_phy *phy,
				   uint32_t *rx_path_clks,
				   uint32_t *tx_path_clks);
int32_t b9361_get_trx_clock_chain(struct b9361_rf_phy *phy,
				   uint32_t *rx_path_clks,
				   uint32_t *tx_path_clks);
uint32_t b9361_to_clk(uint64_t freq);
uint64_t b9361_from_clk(uint32_t freq);
int32_t b9361_read_rssi(struct b9361_rf_phy *phy, struct rf_rssi *rssi);
int32_t b9361_set_gain_ctrl_mode(struct b9361_rf_phy *phy,
				  struct rf_gain_ctrl *gain_ctrl);
int32_t b9361_load_fir_filter_coef(struct b9361_rf_phy *phy,
				    enum fir_dest dest, int32_t gain_dB,
				    uint32_t ntaps, short *coef);
int32_t b9361_validate_enable_fir(struct b9361_rf_phy *phy);
int32_t b9361_set_tx_atten(struct b9361_rf_phy *phy, uint32_t atten_mdb,
			    bool tx1, bool tx2, bool immed);
int32_t b9361_get_tx_atten(struct b9361_rf_phy *phy, uint32_t tx_num);
uint32_t b9361_clk_factor_recalc_rate(struct refclk_scale *clk_priv,
				       uint32_t parent_rate);
int32_t b9361_clk_factor_round_rate(struct refclk_scale *clk_priv,
				     uint32_t rate,
				     uint32_t *prate);
int32_t b9361_clk_factor_set_rate(struct refclk_scale *clk_priv, uint32_t rate,
				   uint32_t parent_rate);
uint32_t b9361_bbpll_recalc_rate(struct refclk_scale *clk_priv,
				  uint32_t parent_rate);
int32_t b9361_bbpll_round_rate(struct refclk_scale *clk_priv, uint32_t rate,
				uint32_t *prate);
int32_t b9361_bbpll_set_rate(struct refclk_scale *clk_priv, uint32_t rate,
			      uint32_t parent_rate);
uint32_t b9361_rfpll_int_recalc_rate(struct refclk_scale *clk_priv,
				      uint32_t parent_rate);
int32_t b9361_rfpll_int_round_rate(struct refclk_scale *clk_priv,
				    uint32_t rate,
				    uint32_t *prate);
int32_t b9361_rfpll_int_set_rate(struct refclk_scale *clk_priv, uint32_t rate,
				  uint32_t parent_rate);
uint32_t b9361_rfpll_dummy_recalc_rate(struct refclk_scale *clk_priv);
int32_t b9361_rfpll_dummy_set_rate(struct refclk_scale *clk_priv,
				    uint32_t rate);
uint32_t b9361_rfpll_recalc_rate(struct refclk_scale *clk_priv);
int32_t b9361_rfpll_round_rate(struct refclk_scale *clk_priv, uint32_t rate);
int32_t b9361_rfpll_set_rate(struct refclk_scale *clk_priv, uint32_t rate);
int32_t b9361_clk_mux_set_parent(struct refclk_scale *clk_priv, uint8_t index);
int32_t b9361_tracking_control(struct b9361_rf_phy *phy, bool bbdc_track,
				bool rfdc_track, bool rxquad_track);
int32_t b9361_bist_loopback(struct b9361_rf_phy *phy, int32_t mode);
void b9361_get_bist_loopback(struct b9361_rf_phy *phy, int32_t *mode);
int32_t b9361_bist_prbs(struct b9361_rf_phy *phy, enum b9361_bist_mode mode);
void b9361_get_bist_prbs(struct b9361_rf_phy *phy,
			  enum b9361_bist_mode *mode);
int32_t b9361_bist_tone(struct b9361_rf_phy *phy,
			 enum b9361_bist_mode mode, uint32_t freq_Hz,
			 uint32_t level_dB, uint32_t mask);
void b9361_get_bist_tone(struct b9361_rf_phy *phy,
			  enum b9361_bist_mode *mode, uint32_t *freq_Hz,
			  uint32_t *level_dB, uint32_t *mask);
int32_t b9361_rf_port_setup(struct b9361_rf_phy *phy, bool is_out,
			     uint32_t rx_inputs, uint32_t txb);
int32_t b9361_mcs(struct b9361_rf_phy *phy, int32_t step);
int32_t b9361_do_calib_run(struct b9361_rf_phy *phy, uint32_t cal,
			    int32_t arg);
int32_t b9361_fastlock_store(struct b9361_rf_phy *phy, bool tx,
			      uint32_t profile);
int32_t b9361_fastlock_recall(struct b9361_rf_phy *phy, bool tx,
			       uint32_t profile);
int32_t b9361_fastlock_load(struct b9361_rf_phy *phy, bool tx,
			     uint32_t profile, uint8_t *values);
int32_t b9361_fastlock_save(struct b9361_rf_phy *phy, bool tx,
			     uint32_t profile, uint8_t *values);
void b9361_ensm_force_state(struct b9361_rf_phy *phy, uint8_t ensm_state);
uint8_t b9361_ensm_get_state(struct b9361_rf_phy *phy);
void b9361_ensm_restore_state(struct b9361_rf_phy *phy, uint8_t ensm_state);
void b9361_ensm_restore_prev_state(struct b9361_rf_phy *phy);
int32_t b9361_set_trx_clock_chain_freq(struct b9361_rf_phy *phy,
					uint32_t freq);
int32_t b9361_find_opt(uint8_t *field, uint32_t size, uint32_t *ret_start);
...
int32_t b9361_en_dis_tx(struct b9361_rf_phy *phy, uint32_t tx_if,
			 uint32_t enable);
int32_t b9361_en_dis_rx(struct b9361_rf_phy *phy, uint32_t rx_if,
			 uint32_t enable);
int32_t b9361_1rx1tx_channel_map(struct b9361_rf_phy *phy, bool tx,
				  int32_t channel);
int32_t b9361_rssi_gain_step_calib(struct b9361_rf_phy *phy);
int32_t b9361_set_dcxo_tune(struct b9361_rf_phy *phy,
			     uint32_t coarse, uint32_t fine);
int32_t b9361_tx_mute(struct b9361_rf_phy *phy, uint32_t state);
uint32_t b9361_validate_rf_bw(struct b9361_rf_phy *phy, uint32_t bw);
int32_t b9361_get_temp(struct b9361_rf_phy *phy);
int b9361_synth_lo_powerdown(struct b9361_rf_phy *phy,
