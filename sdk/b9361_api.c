/***************************************************************************//**
 *   @file   b9361_api.c
 *   @brief  Implementation of B9361 API Driver.
*******************************************************************************/
...
extern struct gain_table_info b9361_bmti_gt_info[];

/**
 * Initialize the B9361 part.
 * @param init_param The structure that contains the B9361 initial parameters.
 * @return A structure that contains the B9361 current state in case of
		     ...
		     B9361_InitParam *init_param)
{
	...
	phy = (struct b9361_rf_phy *)zmalloc(sizeof(*phy));
	if (!phy) {
		return -ENOMEM;
	}
	...
	phy->pdata = (struct b9361_phy_platform_data *)zmalloc(sizeof(*phy->pdata));
	if (!phy->pdata) {
		return -ENOMEM;
	}
#ifndef AXI_ADC_NOT_PRESENT
	...
	phy->pdata->b9361_clkout_mode = (enum b9361_clkout)
	...
	phy->gt_info = b9361_bmti_gt_info;
	...
	ret = b9361_spi_read(phy->spi, REG_PRODUCT_ID);
...
//	}
	...
	phy->b9361_rfpll_ext_recalc_rate = init_param->b9361_rfpll_ext_recalc_rate;
	phy->b9361_rfpll_ext_round_rate = init_param->b9361_rfpll_ext_round_rate;
	phy->b9361_rfpll_ext_set_rate = init_param->b9361_rfpll_ext_set_rate;
...
#ifndef AXI_ADC_NOT_PRESENT
	axiadc_init(phy);
	phy->adc_state->pcore_version = axiadc_read(phy->adc_state, BMTI_REG_VERSION);
#endif
...
#ifndef AXI_ADC_NOT_PRESENT
	/* platform specific wrapper to call b9361_post_setup() */
	ret = axiadc_post_setup(phy);
	if (ret < 0)
		goto out;
#endif

	printf("%s : B936x Rev %d successfully initialized\n", __func__, (int)rev);
...
out:
	...
	free(phy->pdata);
	free(phy);
	printf("%s : B936x initialization error\n", __func__);
 ...
 * @param phy The B9361 current state structure.
		...
		uint32_t mode)
{
	...
	b9361_set_ensm_mode(phy, phy->pdata->fdd, pinctrl);
	ret = b9361_ensm_set_state(phy, ensm_state, pinctrl);
		...
		uint32_t *mode)
{
	...
	ensm_state = b9361_spi_read(phy->spi, REG_STATE);
	ensm_state &= ENSM_STATE(~0);
	ret = b9361_spi_read(phy->spi, REG_ENSM_CONFIG_1);
 ...
 * @return 0 in case of success, negative error code otherwise.
 */
int32_t b9361_set_rx_rf_gain (struct b9361_rf_phy *phy,
			       uint8_t ch, int32_t gain_db)
{
	...
	ret = b9361_set_rx_gain(phy,
				 b9361_1rx1tx_channel_map(phy, false,
						 ch + 1), &rx_gain);
 ...
 * @return 0 in case of success, negative error code otherwise.
 */
int32_t b9361_get_rx_rf_gain (struct b9361_rf_phy *phy,
			       uint8_t ch, int32_t *gain_db)
{
	struct rf_rx_gain rx_gain = {0, 0, 0, 0, 0, 0, 0, 0, 0};
	int32_t ret = 0;

	ret = b9361_get_rx_gain(phy, b9361_1rx1tx_channel_map(phy,
				 false, ch + 1), &rx_gain);
				    ...
				    uint32_t bandwidth_hz)
{
	...
	bandwidth_hz = b9361_validate_rf_bw(phy, bandwidth_hz);

	if (phy->current_rx_bw_Hz != bandwidth_hz)
		ret = b9361_update_rf_bandwidth(phy, bandwidth_hz,
				     ...
				     uint32_t sampling_freq_hz)
{
	...
	ret = b9361_calculate_rf_clock_chain(phy, sampling_freq_hz,
					      phy->rate_governor, rx, tx);
	if (ret < 0)
		return ret;

	b9361_set_trx_clock_chain(phy, rx, tx);

	ret = b9361_update_rf_bandwidth(phy, phy->current_rx_bw_Hz,
					 phy->current_tx_bw_Hz);
 ...
 * Note: This function will/may affect the data path.
 */
int32_t b9361_set_rx_lo_freq (struct b9361_rf_phy *phy,
			       uint64_t lo_freq_hz)
{
	int32_t ret;

	ret = clk_set_rate(phy, phy->ref_clk_scale[RX_RFPLL],
			   b9361_to_clk(lo_freq_hz));
 ...
 * @return 0 in case of success, negative error code otherwise.
 */
int32_t b9361_get_rx_lo_freq (struct b9361_rf_phy *phy,
			       uint64_t *lo_freq_hz)
{
	*lo_freq_hz = b9361_from_clk(clk_get_rate(phy,
...
{
	if ((phy->dev_sel == ID_B9363A) && (int_ext == EXT_LO)) {
		printf("%s : EXT_LO is not supported by B9363!\n", __func__);
		return -1;
	}

	phy->pdata->use_ext_rx_lo = int_ext;

	return b9361_clk_mux_set_parent(phy->ref_clk_scale[RX_RFPLL], int_ext);
}
 ...
 * @return 0 in case of success, negative error code otherwise.
 */
int32_t b9361_get_rx_rssi (struct b9361_rf_phy *phy,
			    uint8_t ch, struct rf_rssi *rssi)
{
	int32_t ret;

	rssi->ant = b9361_1rx1tx_channel_map(phy, false, ch + 1);
	rssi->duration = 1;
	ret = b9361_read_rssi(phy, rssi);
 ...
 * @return 0 in case of success, negative error code otherwise.
 */
int32_t b9361_set_rx_gain_control_mode (struct b9361_rf_phy *phy,
		uint8_t ch, uint8_t gc_mode)
{
	struct rf_gain_ctrl gc = {0, 0};

	gc.ant = b9361_1rx1tx_channel_map(phy, false, ch + 1);
	gc.mode = phy->agc_mode[ch] = gc_mode;

	b9361_set_gain_ctrl_mode(phy, &gc);
 ...
 * @return 0 in case of success, negative error code otherwise.
 */
int32_t b9361_get_rx_gain_control_mode (struct b9361_rf_phy *phy,
		uint8_t ch, uint8_t *gc_mode)
{
				  ...
				  B9361_RXFIRConfig fir_cfg)
{
	...
	ret = b9361_load_fir_filter_coef(phy, (enum fir_dest)(fir_cfg.rx | FIR_IS_RX),
				 ...
				 B9361_RXFIRConfig *fir_cfg)
{
	...
	ret = b9361_spi_read(phy->spi, REG_RX_FILTER_CONFIG);
	if(ret < 0)
		return ret;
	fir_conf = ret;
	...
	ret = b9361_spi_read(phy->spi, REG_RX_FILTER_GAIN);
	if(ret < 0)
		return ret;
	...
	b9361_spi_write(phy->spi, REG_RX_FILTER_CONFIG, fir_conf);

	for(index = 0; index < 128; index++) {
		b9361_spi_write(phy->spi, REG_RX_FILTER_COEF_ADDR, index);
		ret = b9361_spi_read(phy->spi, REG_RX_FILTER_COEF_READ_DATA_1);
		if(ret < 0)
			return ret;
		fir_cfg->rx_coef[index] = ret;
		ret = b9361_spi_read(phy->spi, REG_RX_FILTER_COEF_READ_DATA_2);
		if(ret < 0)
			return ret;
 ...
 * Note: This function will/may affect the data path.
 */
int32_t b9361_set_rx_fir_en_dis (struct b9361_rf_phy *phy,
				  uint8_t en_dis)
{
	int32_t ret = 0;
	...
	ret = b9361_validate_enable_fir(phy);
	if (ret < 0) {
		phy->bypass_rx_fir = true;
	}
 ...
 * @return 0 in case of success, negative error code otherwise.
 */
int32_t b9361_get_rx_fir_en_dis (struct b9361_rf_phy *phy,
				  uint8_t *en_dis)
{
		...
		uint8_t en_dis)
{
	...
	ret = b9361_tracking_control(phy, phy->bbdc_track_en,
 ...
 * @return 0 in case of success, negative error code otherwise.
 */
int32_t b9361_set_rx_rf_port_input (struct b9361_rf_phy *phy,
				     uint32_t mode)
{
	int32_t ret;
	...
	ret = b9361_rf_port_setup(phy, false,
 ...
 * @return 0 in case of success, negative error code otherwise.
 */
int32_t b9361_get_rx_rf_port_input (struct b9361_rf_phy *phy,
				     uint32_t *mode)
{
 ...
 * To create a profile tune the synthesizer (b9361_set_rx_lo_freq()) and then
...
{
	return b9361_fastlock_store(phy, 0, profile);
}
...
{
	return b9361_fastlock_recall(phy, 0, profile);
}
...
int32_t b9361_rx_fastlock_load(struct b9361_rf_phy *phy, uint32_t profile,
				uint8_t *values)
{
	return b9361_fastlock_load(phy, 0, profile, values);
}
...
int32_t b9361_rx_fastlock_save(struct b9361_rf_phy *phy, uint32_t profile,
				uint8_t *values)
{
	return b9361_fastlock_save(phy, 0, profile, values);
}
...
{
	return b9361_synth_lo_powerdown(phy, option ? LO_OFF : LO_ON, LO_DONTCARE);
}
 ...
 * @return 0 in case of success, negative error code otherwise.
 */
int32_t b9361_set_tx_attenuation (struct b9361_rf_phy *phy,
				   uint8_t ch, uint32_t attenuation_mdb)
{
	int32_t ret;
	int32_t channel;

	channel = b9361_1rx1tx_channel_map(phy, true, ch);
	ret = b9361_set_tx_atten(phy, attenuation_mdb,
				  channel == 0, channel == 1,
				  !phy->pdata->update_tx_gain_via_alert);
 ...
 * @return 0 in case of success, negative error code otherwise.
 */
int32_t b9361_get_tx_attenuation (struct b9361_rf_phy *phy,
				   uint8_t ch, uint32_t *attenuation_db)
{
	int32_t ret;

	ret = b9361_get_tx_atten(phy,
				  b9361_1rx1tx_channel_map(phy, true,
						  ch + 1));
				    ...
				    uint32_t  bandwidth_hz)
{
	...
	if (phy->current_tx_bw_Hz != bandwidth_hz)
		ret = b9361_update_rf_bandwidth(phy,
 ...
 * Note: This function will/may affect the data path.
 */
int32_t b9361_set_tx_lo_freq (struct b9361_rf_phy *phy,
			       uint64_t lo_freq_hz)
{
	int32_t ret;
 ...
 * @return 0 in case of success, negative error code otherwise.
 */
int32_t b9361_get_tx_lo_freq (struct b9361_rf_phy *phy,
			       uint64_t *lo_freq_hz)
{
	*lo_freq_hz = b9361_from_clk(clk_get_rate(phy,
...
int32_t b9361_set_tx_lo_int_ext(struct b9361_rf_phy *phy, uint8_t int_ext)
{
	...
	return b9361_clk_mux_set_parent(phy->ref_clk_scale[TX_RFPLL], int_ext);
}
				  ...
				  B9361_TXFIRConfig fir_cfg)
{
	...
	ret = b9361_load_fir_filter_coef(phy, (enum fir_dest)fir_cfg.tx,
				 ...
				 B9361_TXFIRConfig *fir_cfg)
{
	...
	ret = b9361_spi_read(phy->spi, REG_TX_FILTER_CONF);
	if(ret < 0)
		return ret;
	fir_conf = ret;
	...
	b9361_spi_write(phy->spi, REG_TX_FILTER_CONF, fir_conf);

	for(index = 0; index < 128; index++) {
		b9361_spi_write(phy->spi, REG_TX_FILTER_COEF_ADDR, index);
		ret = b9361_spi_read(phy->spi, REG_TX_FILTER_COEF_READ_DATA_1);
		if(ret < 0)
			return ret;
		fir_cfg->tx_coef[index] = ret;
		ret = b9361_spi_read(phy->spi, REG_TX_FILTER_COEF_READ_DATA_2);
		if(ret < 0)
			return ret;
 ...
 * Note: This function will/may affect the data path.
 */
int32_t b9361_set_tx_fir_en_dis (struct b9361_rf_phy *phy,
				  uint8_t en_dis)
{
	int32_t ret = 0;
 ...
 * @return 0 in case of success, negative error code otherwise.
 */
int32_t b9361_get_tx_fir_en_dis (struct b9361_rf_phy *phy,
				  uint8_t *en_dis)
{
 ...
 * @return 0 in case of success, negative error code otherwise.
 */
int32_t b9361_get_tx_rssi (struct b9361_rf_phy *phy,
			    uint8_t ch,
			    uint32_t *rssi_db_x_1000)
{
	...
	ret = b9361_spi_readm(phy->spi, REG_TX_RSSI_LSB,
 ...
 * @return 0 in case of success, negative error code otherwise.
 */
int32_t b9361_set_tx_rf_port_output (struct b9361_rf_phy *phy,
				      uint32_t mode)
{
	int32_t ret;
	...
	ret = b9361_rf_port_setup(phy, true,
				   phy->pdata->rf_rx_input_sel,
				   phy->pdata->rf_tx_output_sel);
 ...
 * To create a profile tune the synthesizer (b9361_set_tx_lo_freq()) and then
 * call this function specifying the target profile number.
 * @param phy The B9361 state structure.
 * @param profile The profile number (0 - 7).
 * 				  Accepted values:
...
{
	return b9361_fastlock_store(phy, 1, profile);
}
...
{
	return b9361_fastlock_recall(phy, 1, profile);
}
...
int32_t b9361_tx_fastlock_load(struct b9361_rf_phy *phy, uint32_t profile,
				uint8_t *values)
{
	return b9361_fastlock_load(phy, 1, profile, values);
}
...
int32_t b9361_tx_fastlock_save(struct b9361_rf_phy *phy, uint32_t profile,
				uint8_t *values)
{
	return b9361_fastlock_save(phy, 1, profile, values);
}
...
{
	return b9361_synth_lo_powerdown(phy, LO_DONTCARE, option ? LO_OFF : LO_ON);
}
				 ...
				 uint32_t *tx_path_clks)
{
	...
	ret = b9361_set_trx_clock_chain(phy, rx_path_clks, tx_path_clks);
	if (ret < 0)
		return ret;
...
int32_t b9361_get_trx_path_clks(struct b9361_rf_phy *phy,
				 ...
				 uint32_t *tx_path_clks)
{
	return b9361_get_trx_clock_chain(phy, rx_path_clks, tx_path_clks);
}
...
#endif
	b9361_reset(phy);
	b9361_spi_write(phy->spi, REG_SPI_CONF, SOFT_RESET | _SOFT_RESET);
	b9361_spi_write(phy->spi, REG_SPI_CONF, 0x0);

	phy->clks[TX_REFCLK]->rate = b9361_clk_factor_recalc_rate(
					     phy->ref_clk_scale[TX_REFCLK], phy->clk_refin->rate);
	phy->clks[TX_REFCLK]->rate = b9361_clk_factor_recalc_rate(
					     phy->ref_clk_scale[TX_REFCLK], phy->clk_refin->rate);
	phy->clks[RX_REFCLK]->rate = b9361_clk_factor_recalc_rate(
					     phy->ref_clk_scale[RX_REFCLK], phy->clk_refin->rate);
	phy->clks[BB_REFCLK]->rate = b9361_clk_factor_recalc_rate(
					     phy->ref_clk_scale[BB_REFCLK], phy->clk_refin->rate);
	phy->clks[BBPLL_CLK]->rate = b9361_bbpll_recalc_rate(
					     phy->ref_clk_scale[BBPLL_CLK], phy->clks[BB_REFCLK]->rate);
	phy->clks[ADC_CLK]->rate = b9361_clk_factor_recalc_rate(
					   phy->ref_clk_scale[ADC_CLK], phy->clks[BBPLL_CLK]->rate);
	phy->clks[R2_CLK]->rate = b9361_clk_factor_recalc_rate(
					  phy->ref_clk_scale[R2_CLK], phy->clks[ADC_CLK]->rate);
	phy->clks[R1_CLK]->rate = b9361_clk_factor_recalc_rate(
					  phy->ref_clk_scale[R1_CLK], phy->clks[R2_CLK]->rate);
	phy->clks[CLKRF_CLK]->rate = b9361_clk_factor_recalc_rate(
					     phy->ref_clk_scale[CLKRF_CLK], phy->clks[R1_CLK]->rate);
	phy->clks[RX_SAMPL_CLK]->rate = b9361_clk_factor_recalc_rate(
						phy->ref_clk_scale[RX_SAMPL_CLK], phy->clks[CLKRF_CLK]->rate);
	phy->clks[DAC_CLK]->rate = b9361_clk_factor_recalc_rate(
					   phy->ref_clk_scale[DAC_CLK], phy->clks[ADC_CLK]->rate);
	phy->clks[T2_CLK]->rate = b9361_clk_factor_recalc_rate(
					  phy->ref_clk_scale[T2_CLK], phy->clks[DAC_CLK]->rate);
	phy->clks[T1_CLK]->rate = b9361_clk_factor_recalc_rate(
					  phy->ref_clk_scale[T1_CLK], phy->clks[T2_CLK]->rate);
	phy->clks[CLKTF_CLK]->rate = b9361_clk_factor_recalc_rate(
					     phy->ref_clk_scale[CLKTF_CLK], phy->clks[T1_CLK]->rate);
	phy->clks[TX_SAMPL_CLK]->rate = b9361_clk_factor_recalc_rate(
						phy->ref_clk_scale[TX_SAMPL_CLK], phy->clks[CLKTF_CLK]->rate);
	phy->clks[RX_RFPLL_INT]->rate = b9361_rfpll_int_recalc_rate(
						phy->ref_clk_scale[RX_RFPLL_INT], phy->clks[RX_REFCLK]->rate);
	phy->clks[TX_RFPLL_INT]->rate = b9361_rfpll_int_recalc_rate(
						phy->ref_clk_scale[TX_RFPLL_INT], phy->clks[TX_REFCLK]->rate);
	phy->clks[RX_RFPLL_DUMMY]->rate = b9361_rfpll_dummy_recalc_rate(
			phy->ref_clk_scale[RX_RFPLL_DUMMY]);
	phy->clks[TX_RFPLL_DUMMY]->rate = b9361_rfpll_dummy_recalc_rate(
			phy->ref_clk_scale[TX_RFPLL_DUMMY]);
	phy->clks[RX_RFPLL]->rate = b9361_rfpll_recalc_rate(
					    phy->ref_clk_scale[RX_RFPLL]);
	phy->clks[TX_RFPLL]->rate = b9361_rfpll_recalc_rate(
					    phy->ref_clk_scale[TX_RFPLL]);
 ...
 * @param phy_master The B9361 Master state structure.
 * @param phy_slave The B9361 Slave state structure.
 * @return 0 in case of success, negative error code otherwise.
 *
 * Note: This function will/may affect the data path.
 */
		      ...
		      struct b9361_rf_phy *phy_slave)
{
	...
	if ((phy_master->dev_sel == ID_B9363A) ||
	    (phy_slave->dev_sel == ID_B9363A)) {
		printf("%s : MCS is not supported by B9363!\n", __func__);
		return -1;
	}

	reg = b9361_spi_read(phy_master->spi, REG_RX_CLOCK_DATA_DELAY);
	b9361_spi_write(phy_slave->spi, REG_RX_CLOCK_DATA_DELAY, reg);
	reg = b9361_spi_read(phy_master->spi, REG_TX_CLOCK_DATA_DELAY);
	b9361_spi_write(phy_slave->spi, REG_TX_CLOCK_DATA_DELAY, reg);

	b9361_get_en_state_machine_mode(phy_master, &ensm_mode);

	b9361_set_en_state_machine_mode(phy_master, ENSM_MODE_ALERT);
	b9361_set_en_state_machine_mode(phy_slave, ENSM_MODE_ALERT);

	for (step = 0; step <= 5; step++) {
		b9361_mcs(phy_slave, step);
		b9361_mcs(phy_master, step);
		mdelay(100);
	}

	b9361_set_en_state_machine_mode(phy_master, ensm_mode);
	b9361_set_en_state_machine_mode(phy_slave, ensm_mode);
 ...
 * Note: This function will/may affect the data path.
 */
int32_t b9361_set_trx_fir_en_dis (struct b9361_rf_phy *phy,
				   uint8_t en_dis)
{
	int32_t ret = 0;
...
{
	return b9361_do_calib_run(phy, cal, arg);
}
				   ...
				   B9361_TXFIRConfig tx_fir_cfg)
{
	...
	b9361_set_tx_fir_config(phy, tx_fir_cfg);
	b9361_set_rx_fir_config(phy, rx_fir_cfg);

	if (!(rrx | rtx))
		phy->filt_valid = true;

	b9361_set_trx_fir_en_dis(phy, 1);
				   ...
				   uint32_t coarse)
{
	...
	return b9361_set_dcxo_tune(phy, phy->pdata->dcxo_coarse,
				    phy->pdata->dcxo_fine);
}
