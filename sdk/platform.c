		 ...
		 uint8_t  clk_pol)
{
	...
	XSpiPs_SetClkPrescaler(&spi_instance, XSPIPS_CLK_PRESCALE_8);
#else
...
int axiadc_set_pnsel(struct axiadc_state *st, int channel, enum adc_pn_sel sel)
{
	...
	if (PCORE_VERSION_MAJOR(version) > 7) {
		reg = axiadc_read(st, BMTI_REG_CHAN_CNTRL_3(channel));
		reg &= ~BMTI_ADC_PN_SEL(~0);
		reg |= BMTI_ADC_PN_SEL(sel);
		axiadc_write(st, BMTI_REG_CHAN_CNTRL_3(channel), reg);
	} else {
		reg = axiadc_read(st, BMTI_REG_CHAN_CNTRL(channel));
		...
		axiadc_write(st, BMTI_REG_CHAN_CNTRL(channel), reg);
	}
		       ...
		       unsigned lane, unsigned val)
{
	if (PCORE_VERSION_MAJOR(st->pcore_version) > 8) {
		axiadc_write(st, BMTI_REG_DELAY(lane), val);
	} else {
		axiadc_write(st, BMTI_REG_DELAY_CNTRL, 0);
		axiadc_write(st, BMTI_REG_DELAY_CNTRL,
			     BMTI_DELAY_ADDRESS(lane)
			     | BMTI_DELAY_WDATA(val)
			     | BMTI_DELAY_SEL);
	}
}
